	...
	export default{
		...
		methods:{
			...
			starTing(screen,nums){
				...
				.then((res)=>{
					this.$store.commit('screenmuta',res.data)// 筛选到的存储到vuex数据仓库
				})
				.catch((err)=>{
					console.log(err)
				})
			...
			compLete(){
				...
				.then((res)=>{
					this.$store.commit('screenmuta',res.data)// 存储到vuex数据仓库
				})
				.catch((err)=>{
					console.log(err)
				})
		...
		computed:{
			emptyIng(){// 如果用户没有点击任何筛选条件，完成按钮应该被禁用
				let obj = JSON.stringify(this.multiobj) == '{}'// this.multiobj如果是空对象，禁止点击完成按钮，反之。。。。
				if(obj == true){
					this.flag = false
				}else{
					this.flag = true
