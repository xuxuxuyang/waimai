...
<script>
	...
	import tips from '../../element/tips.vue'// 引入反馈组件
	var {log} = console // es6解构log
	export default{
		components:{
			tips
		},
		...
		methods:{
			...
			getStorage(searchkey){// 本地存入搜索历史
				let seararray = uni.getStorageSync('search_key') || []// 先取出本地缓存的数据
				seararray.unshift(searchkey)		// 要存入一个数组array
				uni.setStorageSync('search_key', seararray);
			},
			setStorage(){// 取出本地缓存的数据
				let setdata = uni.getStorageSync('search_key')
				let setdataarr = Array.from(new Set(setdata))// 数组去重
				if(setdataarr == ''){
					this.ifhistory = false
				}else{
			...
			removeStorage(){// 手动清除搜索历史
				uni.removeStorageSync('search_key');
				this.setStorage()
			},
			menubtn(item){// 点击搜索历史的搜索
				this.ifhistory = false
				this.searchData(item)
			},
			searchData(searchkey){ //发送请求搜索
				let data = {
